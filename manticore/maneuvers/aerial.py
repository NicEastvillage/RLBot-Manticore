...
class AerialManeuver(Maneuver):

    def __init__(self, ball_location: Vec3, intercept_time: float, do_jump: bool, do_second_jump: bool = False, target_rot: Mat33 = None):
        super().__init__()
        self.hit_pos = ball_location
        self.intercept_time = intercept_time  # absolute time, e.g. 3m32s78ms
        self.target_rot = target_rot
        self.jumping = do_jump
    ...
    def exec(self, bot):
        ...
        if self.jumping:
            ...
            if jump_elapsed < JUMP_MAX_DUR:
                ...
                controls.jump = False
                if self.do_second_jump:
                    if self.jump_pause_counter < 3:
                        # Do a 3-tick pause between jumps
    ...
    def is_viable(self, obj, rot, boost, current_time: float):
        """
        obj: car or DummyObject with position and velocity
        """
        up = rot.up
        T = self.intercept_time - current_time
        xf = obj.pos + obj.vel * T + 0.5 * GRAVITY * T ** 2
        vf = obj.vel + GRAVITY * T
        ...
        phi = angle_between(dir, rot.forward)
        ...
        enough_boost = boost_estimate < boost
